...
int game_dijkstra_search(const game_info_t* info,
	...
	while(!heapq_empty(&pq) && result == SEARCH_IN_PROGRESS){
		...
		tree_node_t* curr_node = heapq_deque(&pq);

		//Get next color to explore its 4 directions
		int color = game_next_move_color(info, &(curr_node -> state));
		...
		for (int dir = DIR_LEFT; dir <= DIR_DOWN; dir++){       
			...
			if(game_can_move(info, &(curr_node -> state), color, dir)){

				//Create child node
				tree_node_t* child = node_create(curr_node, info, &(curr_node -> state));
	...
	if (heapq_empty(&pq) && result == SEARCH_IN_PROGRESS) {
